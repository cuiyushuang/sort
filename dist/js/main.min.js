...
$$1('createArr').onclick = function () {
  var val1 = $$1('in').value;
  var val2 = $$1('import').value;
  if (val2) {
    var pcs = $$1('import').value.split(',');
    var arr = pcs;
    show$1(arr);
  } else {
    var _pcs = val1;
    var _arr = create$1(parseInt(_pcs));
    show$1(_arr);
  }
};
$$1('select').onclick = function () {
  selectSort();
};
//计算最大高度
...
function addClass$1(ele, cls) {
  if (!hasClass$1(ele, cls)) {
    ele.className = ele.className == '' ? cls : ele.className + ' ' + cls;
  }
}
...
//产生柱状图
function show$1(data) {
  var maxWidth = 800;
  var maxHeight = 450;
  var maxNum = getMax$1(data);
  //计算比例
  var percent = maxHeight / maxNum;
  //柱子间隔
  var space = maxWidth / (data.length * 2 + 1);
...
async function selectSort() {
  ...
  for (var i = 0; i < len - 1; i++) {
    minIndex = i;
    addClass$1(data[minIndex], 'light');
    for (var j = i + 1; j < len; j++) {
      //addClass(data[j],'end');
      if (parseInt(arrC[j]) < parseInt(arrC[minIndex])) {
        //寻找最小的数
        minIndex = j; //将最小数的索引保存
      }
    }
    swap$1(data[i], data[minIndex]);
    var _temp = data[i];
    data[i] = data[minIndex];
    data[minIndex] = _temp;
    addClass$1(data[i], 'light1'); //橙色
    data.forEach(function (v, i) {
      arrC[i] = v.innerHTML;
    });
    await sleep$1(2000);
  ...
  for (var _i = 0; _i < data.length; _i++) {
    addClass$1(data[_i], 'light1');
  }
}

function swap$1(ele1, ele2) {
  var dis1 = ele1.offsetLeft;
  var dis2 = ele2.offsetLeft;
  var d1 = dis1;
  var d2 = dis2;
